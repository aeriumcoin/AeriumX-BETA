...
// Copyright (c) 2018 The Altair2 developers
...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::ATR)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sATRPercentage, QString& szATRPercentage)
{
    int nPrecision = 2;
    ...
    szATRPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sATRPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount atrAvailableBalance = balance - immatureBalance - nLockedBalance;
    ...
    // Combined balances
    CAmount availableTotalBalance = atrAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // ATR labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, atrAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelATRPercent->setText(sPercentage);
    ui->labelzATRPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zATR.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    bool fEnableZeromint = GetBoolArg("-enablezeromint", true);
    int nZeromintPercentage = GetArg("-zeromintpercentage", 10);
    if (fEnableZeromint) {
        automintHelp += tr("AutoMint is currently enabled and set to ") + QString::number(nZeromintPercentage) + "%.\n";
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in altair2.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in altair2.conf");
    }
    ...
    bool showATRAvailable = settingShowAllBalances || atrAvailableBalance != nTotalBalance;
    bool showWatchOnlyATRAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showATRPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyATRPending = watchUnconfBalance != 0;
    bool showATRLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyATRLocked = nWatchOnlyLockedBalance != 0;
    bool showImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = watchImmatureBalance != 0;
    bool showWatchOnly = nTotalWatchBalance != 0;
    ui->labelBalance->setVisible(showATRAvailable || showWatchOnlyATRAvailable);
    ui->labelBalanceText->setVisible(showATRAvailable || showWatchOnlyATRAvailable);
    ui->labelWatchAvailable->setVisible(showATRAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showATRPending || showWatchOnlyATRPending);
    ui->labelPendingText->setVisible(showATRPending || showWatchOnlyATRPending);
    ui->labelWatchPending->setVisible(showATRPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showATRLocked || showWatchOnlyATRLocked);
    ui->labelLockedBalanceText->setVisible(showATRLocked || showWatchOnlyATRLocked);
    ui->labelWatchLocked->setVisible(showATRLocked && showWatchOnly);
    ...
    bool showzATRAvailable = settingShowAllBalances || zerocoinBalance != matureZerocoinBalance;
    bool showzATRUnconfirmed = settingShowAllBalances || unconfirmedZerocoinBalance != 0;
    bool showzATRImmature = settingShowAllBalances || immatureZerocoinBalance != 0;
    ui->labelzBalanceMature->setVisible(showzATRAvailable);
    ui->labelzBalanceMatureText->setVisible(showzATRAvailable);
    ui->labelzBalanceUnconfirmed->setVisible(showzATRUnconfirmed);
    ui->labelzBalanceUnconfirmedText->setVisible(showzATRUnconfirmed);
    ui->labelzBalanceImmature->setVisible(showzATRImmature);
    ui->labelzBalanceImmatureText->setVisible(showzATRImmature);
    bool showPercentages = ! (zerocoinBalance == 0 && nTotalBalance == 0);
    ui->labelATRPercent->setVisible(showPercentages);
    ui->labelzATRPercent->setVisible(showPercentages);
