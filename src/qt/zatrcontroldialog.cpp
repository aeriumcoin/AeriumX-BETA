...
#include "ui_zatrcontroldialog.h"
...
std::list<std::string> ZATRControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZATRControlDialog::listMints;

ZATRControlDialog::ZATRControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZATRControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZATRControlDialog::~ZATRControlDialog()
{
    delete ui;
}

void ZATRControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZATRControlDialog::updateList()
{
...
void ZATRControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZATRControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZATR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZATRControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZATRControlDialog::GetSelectedMints()
{
...
void ZATRControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
