...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::ATR);
    ...
    if (!settings.contains("nAnonymizeAltair2Amount"))
        settings.setValue("nAnonymizeAltair2Amount", 1000);

    nAnonymizeAltair2Amount = settings.value("nAnonymizeAltair2Amount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeAltair2Amount"))
        SoftSetArg("-anonymizealtair2amount", settings.value("nAnonymizeAltair2Amount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in altair2.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAltair2Amount:
            return QVariant(nAnonymizeAltair2Amount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeAltair2Amount:
            nAnonymizeAltair2Amount = value.toInt();
            settings.setValue("nAnonymizeAltair2Amount", nAnonymizeAltair2Amount);
            emit anonymizeAltair2AmountChanged(nAnonymizeAltair2Amount);
            break;
        case CoinControlFeatures:
